<template>
    <el-container>
        ...
        <el-main>
            <page-content :ref="'bc_list'" body_key="biddings" :search_input="search_info"
                :search_key="['stuff.name']" :req_url="'/bid/get_all_created'" :enable="true">
                <template #default="slotProps">
                    <el-table :data="slotProps.content" :table-layout="table_layout">
                        <el-table-column label="展开" type="expand">
                            <template #default="scope">
                                <el-table :data="scope.row.bidding_turns" :table-layout="table_layout" size="mini"
                                    style="padding-left: 50px;">
                                    <el-table-column label="竞价项" type="expand" width="120">
                                        <template #default="scope">
                                            <el-table :data="scope.row.bidding_items" size="mini"
                                                :table-layout="table_layout" style="padding-left: 50px;">
                                                <el-table-column prop="rbac_user.company.name"
                                                    label="竞价公司"></el-table-column>
                        ...
                        <el-table-column v-permission="['bid']" label="操作" width="300">
                            <template slot-scope="scope">
                                <el-button v-if="scope.row.status == 0 && scope.row.bidding_turns.length == 0"
                                    type="primary" size="mini" @click="handle_start_bid(scope.row)">开启</el-button>
                                <el-button
                                    v-else-if="scope.row.status == 0 && scope.row.bidding_turns.length != scope.row.total_turn"
                                    type="warning" size="mini" @click="handle_next_bid(scope.row)">开启下一轮</el-button>
                                <el-button v-if="scope.row.status == 0" type="danger" size="mini"
                                    @click="stop_bid(scope.row)">停止</el-button>
            ...
            <el-dialog title="开启竞价" :visible.sync="show_start_bid" width="600px" @close="show_start_bid = false">
                <el-form ref="start_bid" label-width="120px">
                    <el-form-item label="时间范围">
                        <el-date-picker v-model="bt_time_range" format="yyyy-MM-dd HH:mm" value-format="yyyy-MM-dd HH:mm" type="datetimerange" range-separator="至"
                            start-placeholder="开始日期" end-placeholder="结束日期">
                        </el-date-picker>
                    </el-form-item>
                    <el-form-item label="客户范围">
                        <page-select v-model="selected_companies" placeholder="点击选择客户"
                            :fetch-function="get_related_companies" :params="{ stuff_id: new_bc.stuff_id }"
                            mode="pagenation" multiple @change="handleCustomerSelect">
                            <template #option-list="{ options }">
                                <el-option v-for="item in options" :key="item.id"
                                    :label="`${item.company.name} - ${item.rbac_users[0].name}`"
                                    :value="item.rbac_users[0].id">
                                </el-option>
                            </template>
                        </page-select>
                    </el-form-item>
    ...
    data: function () {
        return {
            ...
            rules: {
                ...
                max: [
                    { required: true, message: '输入最高出价', trigger: 'change' },
                ],
                min: [
                    { required: true, message: '输入最低出价', trigger: 'change' },
                ],
                stuff_name: [
                    { required: true, message: '选择物料', trigger: 'change' }
                ],
                total: [
                    { required: true, message: '输入总量', trigger: 'change' },
                ],
                total_turn: [
