...
static void set_wait_cycle(std::ostream &out, std::vector<std::string> _params)
{
    if (_params.size() != 1)
    {
        out << "参数错误" << std::endl;
    ...
    else
    {
        ...
        try
        {
            running_rule tmp;
            client->get_rule(tmp);
            tmp.wait_cycle = atoi(_params[0].c_str());
            client->set_rule(tmp);
        }
        catch (const gen_exp &e)
        {
...
static void set_min_weight(std::ostream &out, std::vector<std::string> _params)
{
    if (_params.size() != 1)
    {
        out << "参数错误" << std::endl;
    ...
    else
    {
        ...
        try
        {
            running_rule tmp;
            client->get_rule(tmp);
            tmp.min_weight = atof(_params[0].c_str());
            client->set_rule(tmp);
        }
        catch (const gen_exp &e)
        {
...
std::string rule_cli::make_bdr()
{
    ...
    if (tmp.wait_cycle != 0)
    {
        ret.push_back("set_wait_cycle " + std::to_string(tmp.wait_cycle));
    }
    if (tmp.min_weight != 0)
    {
        ret.push_back("set_min_weight " + std::to_string(tmp.min_weight));
    }
