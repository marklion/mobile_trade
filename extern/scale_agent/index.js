...
class output_helper {
  ...
  async apply(serial, script, baudRate = 9600) {
    ...
    if (typeof fn !== 'function') {
      throw new Error('script 必须是函数，例如: function (array) { return array[0] + array[1]; }');
    }
    this._parse = fn;
    ...
    if (!pathName) {
      console.log('[output_helper] 串口为空，仅提供 HTTP 服务');
      return;
    }
    ...
    this._port.on('error', (err) => console.error('[output_helper] 串口错误:', err.message));
    this._port.on('data', (chunk) => this._onData(chunk));
    await new Promise((resolve, reject) => {
      this._port.open((err) => (err ? reject(err) : resolve()));
    });
    console.log(`[output_helper] 串口已打开: ${pathName}, 波特率: ${baudRate}`);
  }
...
async function main() {
  ...
  const helper = new output_helper();
