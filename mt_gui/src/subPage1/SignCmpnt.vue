...
<view class="signature-config-wrap">
    ...
    <view v-if="imgUrl" style="margin-top: 32rpx; text-align: center;">
        ...
        <image :src="imgUrl" mode="widthFix" style="width: 100%; max-width: 400px; margin-top: 16rpx; border: 1px solid #eee;" />
    </view>
</view>
</template>
...
export default {
    ...
    data() {
        return {
            ...
            get_sign_func: async ()=> {
                ...
                if (this.open_id)
                {
                    ret = await this.$send_req('/global/get_user_signature', {open_id:this.open_id});
                }
                else
                {
                    ret = await this.$send_req('/sc/get_user_signature');
                }
                return ret;
            },
            update_sign_func: async (filePath)=> {
                if (this.open_id)
                {
                    await this.$send_req('/global/update_user_signature', {
    ...
    methods: {
        ...
        async loadUserSignature() {
            try {
                // 获取用户已有的签名
                const result = await this.get_sign_func();
        ...
        async saveSignatureToUser(filePath) {
            await this.update_sign_func(filePath);
        },
        async handleClear() {
            try {
                await this.update_sign_func('');
                this.imgUrl = '';
                uni.showToast({
                    title: '签名已清除',
                    icon: 'success'
