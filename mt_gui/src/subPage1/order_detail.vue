<template>
<view>
    <scroll-view v-if="loaded" style="height: 100vh;" show-scrollbar scroll-y>
    ...
    <view v-else style="padding: 40rpx;text-align:center;color:#999;">加载中...</view>
    ...
    <measurement ref="measurement" :focus_plan="focus_plan" @refresh="reload_plan"></measurement>
...
export default {
    ...
    data: function () {
        return {
            ...
            focus_plan: {
                behind_vehicle: { id: 0, plate: '' },
                main_vehicle: { id: 0, plate: '' },
                driver: { id: 0, name: '', phone: '' },
                company: { id: 0, name: '' },
                stuff: { id: 0, name: '', company: { id: 0, name: '' }, concern_fapiao: false, manual_weight: false },
                rbac_user: { id: 0, name: '', phone: '' },
                plan_histories: [],
                status: 0,
                is_buy: false,
                id: 0,
    ...
    methods: {
        ...
        apply_plan: async function (plan) {
            this.focus_plan = plan;
            await this.load_contract(plan);
            this.loaded = true;
            uni.setNavigationBarTitle({ title: '订单' + plan.id });
        },
            ...
            for (let i = 0; i < roles.length; i++) {
                ...
                try {
                    ...
                    if (resp && resp.plan) {
                        ...
                        await this.apply_plan(resp.plan);
                        return;
                    }
                } catch (e) {
                    console.log(e);
