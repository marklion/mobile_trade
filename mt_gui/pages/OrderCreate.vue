<template>
    <view>
        ...
        <u-cell title="卖方" :is_link="saler_name == '(未指定)'" :value="saler_name?saler_name:'(未指定)'"
            @click="show_select_company= true"></u-cell>
        ...
        <fui-form ref="plan_form" :model="plan">
            ...
            <view style="display:flex; justify-content: center;">
                ...
                <fui-button v-if="!type_define.is_sale" text="我要代提" btnSize="small" type="primary"
                    @click="prepare_proxy_buy"></fui-button>
            </view>
            <fui-grid :columns="2" :square="false">
                <fui-grid-item v-for="(single_v, index) in vehicles" :key="index">
                    <view style="border:solid;">
                        <fui-preview bdSize="26" :isBorder="false" :previewData="make_vc_show(single_v)"
                            @click="remove_vehicle(index)"></fui-preview>
                    </view>
                </fui-grid-item>
            </fui-grid>
            <fui-divider></fui-divider>
                ...
                <fui-form-item asterisk label="主车牌" :rules="rules[0]" prop="main_vehicle_plate">
                    <fui-input :padding="[0]"  v-model="new_vehicle.main_vehicle_plate">
                        <fui-button btnSize="mini" type="purple" text="选择"
                            @click="show_pick_vehicles = true"></fui-button>
                    </fui-input>
                </fui-form-item>
