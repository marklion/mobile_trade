<template>
    <view>
        ...
        <list-show full ref="contracts" v-model="data2show" :fetch_function="get_sale_contract" height="85vh"
            style="background-color: aliceblue;" search_key="search_cond" :fetch_params="[cur_urls]">
            <fui-card full :margin="['20rpx', '0rpx']" v-for="item in data2show" :key="item.id" size="large"
                :title="item.company.name" color="black" :tag="'￥' + item.balance">
                <view style="padding: 0 20rpx;">
                    <module-filter require_module="sale_management">
                        <view style="display:flex; flex-wrap: wrap;" v-if="cur_urls.need_su || cur_urls.buy_setting">
                            <fui-tag v-for="(single_stuff, index) in item.stuff" :key="index" theme="plain" originLeft
                                :scaleRatio="0.8" type="purple">
                                {{ single_stuff.name }}
                                <fui-icon name="close" size="32"
                                    @click="prepare_unstuff(item, single_stuff)"></fui-icon>
                            </fui-tag>
                            <view style="display:flex; flex-wrap: wrap;" v-if="cur_urls.need_su">
                                <fui-tag v-for="(single_user) in item.rbac_users" :key="single_user.id" theme="plain"
                                    originLeft :scaleRatio="0.8" type="success">
                                    {{ single_user.name ? single_user.name + '|' + single_user.phone : single_user.phone
									}}
                                    <fui-icon name="close" size="32"
                                        @click="prepare_unauth(item, single_user)"></fui-icon>
                                </fui-tag>
                            </view>
                        </view>
                        <view style="display:flex; flex-wrap: wrap;" v-if="cur_urls.need_su || cur_urls.buy_setting">
                            <fui-tag text="新增物料" :scaleRatio="0.8" originLeft type="purple"
                                @click="prepare_add_stuff(item)"></fui-tag>
                            <fui-tag v-if="cur_urls.need_su" :scaleRatio="0.8" originLeft type="success" text="新增授权"
                                @click="prepare_auth(item)"></fui-tag>
                        </view>
                    </module-filter>
                    <view>
                    ...
                    <view style="display:flex; flex-wrap: wrap;">
                        <module-filter require_module="cash" v-if="cur_urls.need_su">
                            <fui-tag :scaleRatio="0.8" originLeft type="primary" text="充值"
                                @click="prepare_charge(item)"></fui-tag>
                            <fui-tag :scaleRatio="0.8" originLeft type="warning" text="充值记录"
                                @click="prepare_charge_history(item)"></fui-tag>
                        </module-filter>
                        <fui-tag v-if="cur_urls.motive" :scaleRatio="0.8" originLeft type="danger" text="删除"
                            @click="prepare_del(item)"></fui-tag>
                    </view>
                </view>
        ...
        <fui-modal width="600" :show="show_add_contract" @click="add_contract" v-if="show_add_contract">
            <fui-form ref="add_contract" top="100">
                <fui-input label="客商" borderTop placeholder="点击选择客商" v-model="company_name" disabled
                    @click="show_customers = true"></fui-input>
                <fui-input label="开始时间" borderTop disabled placeholder="点击选择时间范围" v-model="new_contract.begin_time"
                    @click="show_date_range = true"></fui-input>
                <fui-input label="结束时间" borderTop disabled placeholder="点击选择时间范围" v-model="new_contract.end_time"
                    @click="show_date_range = true"></fui-input>
                <fui-input label="客商编码" borderTop placeholder="请输入客商编码"
                    v-model="new_contract.customer_code"></fui-input>
        ...
        <fui-bottom-popup :show="show_customers" @close="show_customers = false">
            <fui-list>
                <list-show v-model="customers_data2show" :fetch_function="get_customers" search_key="name"
                    height="40vh">
                    <fui-list-cell arrow v-for="item in customers_data2show" :key="item.id"
                        @click="select_company(item)">
                        {{ item.name }}
                    </fui-list-cell>
                </list-show>
            </fui-list>
        </fui-bottom-popup>
        <fui-date-picker range :show="show_date_range" type="3" :value="new_contract.begin_time"
            :valueEnd="new_contract.end_time" @change="set_date_range"
            @cancel="show_date_range = false"></fui-date-picker>
        <fui-bottom-popup :show="show_add_stuff" @close="show_add_stuff = false">
            <fui-list>
                <list-show ref="stuff_got" v-model="stuff_data2show" :fetch_function="get_stuff"
                    :fetch_params="[cur_urls.buy_setting]" search_key="name" height="40vh">
                    <fui-list-cell arrow v-for="item in stuff_data2show" :key="item.id"
                        @click="add_stuff2contract(item)">
                        {{ item.name }}
                    </fui-list-cell>
                </list-show>
            </fui-list>
        </fui-bottom-popup>
        <fui-modal width="600" :descr="'确定要取消' + focus_item.company.name + '关注' + focus_stuff.name + '吗？'"
            v-if="show_del_stuff" :show="show_del_stuff" @click="del_stuff">
        </fui-modal>

        <fui-modal width="600" :show="show_add_auth" v-if="show_add_auth" maskClosable @click="add_auth"
            @cancel="close_add_auth">
            <fui-form ref="add_auth" top="100">
                <fui-input required label="用户手机号" maxlength="11" borderTop placeholder="请输入手机号"
                    v-model="phone"></fui-input>
            </fui-form>
        </fui-modal>
        <fui-modal width="600" :descr="'确定要取消授权' + focus_user.phone + '吗？'" :show="show_unauth" v-if="show_unauth"
            @click="unauth_user">
        </fui-modal>
        <fui-modal width="600" :descr="'确定要删除' + focus_item.company.name + '吗？'" :show="show_del" v-if="show_del"
            @click="del_contract">
        </fui-modal>
        ...
        <fui-bottom-popup :show="show_charge_history" @close="show_charge_history = false">
            <fui-list>
                <list-show v-model="histories_data2show" ref="history" :fetch_function="get_history"
                    :fetch_params="[focus_item.id]" search_key="search_cond" height="40vh">
                    <u-cell v-for="item in histories_data2show" :key="item.id" :title="item.operator"
                        :value="'￥' + item.cash_increased.toFixed(2)">
                        <view slot="label">
